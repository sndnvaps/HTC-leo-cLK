...
#define PART_DEFAULT		"lk=5,recovery=8,boot=5,misc=1,system=150,userdata=0,cache=5"
#define PART_DEFAULT_EXT_ON	"lk=5,recovery=8,boot=5,misc=1,system=150,userdata=0,cache=192!"
...
int FillRecoveryPart(const char* recoveryFile)
{
	...
	{
		fprintf( stderr, "Too many partitions, max=%ld\n", ARRAYSIZE( vparts.pdef ) );
		return -1;
	}
...
int AddPartImage(const char* fileName, int length)
{
	...
	{
		fprintf( stderr, "Too many ROM partitions, max=%ld\n", ARRAYSIZE( data.parts ) );
		return -1;
	}
...
int main(int argc, char* argv[])
{
	...
	printf( "=== Fixing warnings + some changes by dennes544\n" );
	printf( "\n" );
	if ( argc < 2 )
	{
